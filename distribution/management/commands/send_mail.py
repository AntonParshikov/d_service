...
def frequency_check(distribution):
    """
    Проверяет периодичность оправки сообщений
    """
    next_try = None
...
def start_check(distribution):
    """
    Проверяет дату начала рассылки для ее активации
    """
...
def finish_check(distribution):
    """
    Проверяет дату окончания рассылки для ее отключения
    """
    for user in distribution.distribution_client.all():
        sendmail(distribution.message, user)
...
def distribution_check():
    """
    Проверяет все рассылки на сроки этой рассылки и отправляет сообщения
    """
    for distribution in Distribution.objects.all():
        try:
            ...
            distribution.feedback = 'Рассылка не удалась'
